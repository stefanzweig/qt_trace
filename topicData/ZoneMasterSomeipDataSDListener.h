...
class SomeipSDListener : public virtual QObject, public DataReaderListener
{
    Q_OBJECT
public:
                ...
                if (info.valid_data)
                    {
                        ...
                        for (int i = 0; i < lin_messages_.len(); i++) {
                            ...
                            lin_Frame lf;
                            lf.ID = msg.id();
                            emit ItemUpdate_internal_someip_sd(msg);
                        }
                    }
            }
    }
...
signals:
    void ItemUpdate_internal_someip_sd(linMessage lm);
};
