...
class SomeipEthFrameListener : public virtual QObject, public DataReaderListener
{
    Q_OBJECT
public:

    SomeipEthFrameListener()
    {
    }
                ...
                if (info.valid_data)
                    {
                        ...
                        for (int i = 0; i < lin_messages_.len(); i++) {
                            ...
                            lin_Frame lf;
                            lf.ID = msg.id();
                            emit ItemUpdate_internal_eth_frame(msg);
                        }
                    }
            }
    }
...
signals:
    void ItemUpdate_internal_eth_frame(linMessage lf);
};
