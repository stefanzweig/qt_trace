...
void QtWidgetsApplication1::setDefaultHeader()
{
	bool canlin_mod = false;
	bool someip_mod = false;
	CurrentHeader = initialHeader;
	for (QString mod : monitor_modules)
	{
		qDebug() << "Loaded module ->" << mod;
		if (mod.isEmpty()) continue;
		if (mod.toLower().contains("can") || mod.toLower().contains("lin"))
		{
			canlin_mod = true;
			break;
		}
...
void scrollToCenter(QTreeWidget* treeWidget, QTreeWidgetItem* item) {
	...
	QRect viewportRect = treeWidget->viewport()->rect();
...
void QtWidgetsApplication1::current2center()
{
	...
	//if (itemToScrollTo) {
	//    scrollToCenter(ui.treetrace, itemToScrollTo);
	//}
	...
	QSet<Found_Item*>& visited)
{
	...
	for (const QString& target : targets) {
		for (int column = 0; column < root->columnCount(); ++column) {
			if (root->text(column).toLower().contains(target.toLower())) {
				...
				{
					if (v->item == root && v->col == column)
						item_visited = true;
				}
				if (!item_visited) {
					Found_Item* it = new Found_Item();
					it->item = root;
					it->col = column;
					queue.enqueue(it);
					visited.insert(it); // record the found one.
				}
			}
		}
	}
	...
	for (int i = 0; i < root->childCount(); ++i) {
		searchTree_bfs(root->child(i), targets, queue, visited);
	}
}
...
void QtWidgetsApplication1::show_fullpage_with_findings()
{
	...
	if (found_queue.isEmpty() || search_str != last_findstring) { reset_find_status(); }

	enter_find_status();
	form_conditions_simple(search_str);
	resetInvisibles();

	// this visited set works with the bfs algorithm.
	// 2024-11-04 14:18:30
	QSet<Found_Item*> visited;
	...
	for (int i = 0; i < child_count; ++i) {
		...
		for (const int& key : list_map.keys()) {
			...
			if (full_search) {
				...
				searchTree_bfs(item, values, found_queue, visited);
				hidden = !matched;
			}
			else {
...
void QtWidgetsApplication1::highlight_previous_item(QTreeWidgetItem* item)
{
	...
	if (parent) {
		...
		if (prevSibling)
			...
			ui.treetrace->setCurrentItem(parent);
	}
	else {
...
void QtWidgetsApplication1::highlight_next_item(QTreeWidgetItem* item)
{
	...
	if (parent) {
		...
		if (nextSibling)
			...
			if (firstChild)
				ui.treetrace->setCurrentItem(firstChild);
			else {
				...
				if (currentItem) {
					QTreeWidget* treeWidget = item->treeWidget();
					nextSibling = treeWidget->topLevelItem(treeWidget->indexOfTopLevelItem(currentItem) + 1);
					if (nextSibling) {
